...
public class RunWiki {
	...
	public void run() {
		...
		try {
			...
			SingleStrategyChoice wsc = (SingleStrategyChoice) logootEngine
					.getStrategyChoice();
			...
			while (wsc.getSpectrum().get(first).getNext() != null) {
				...
				for (int j = 0; j < first.size(); ++j) {
					tempsum += logootEngine.getBase().getBase(j+1).bitLength();
				}
	...
	public static Float[] avgAndMaxBitSizeWeiss(List<Positions> logootIdTable,
		...
		Float[] result = { 0f, 0f };
		for (Positions p : logootIdTable) {
			result[0] += (float) (64 * p.size()) / logootIdTable.size();

			if (64 * p.size() > result[1]) {
				result[1] = (float) 64 * p.size();
			}
		}
		return result;
	}
	...
	public static Float[] avgAndMaxBitSizeDouble(List<Positions> logootIdTable,
		...
		for (Positions p : logootIdTable) {
			...
			for (int i = 0; i < p.size(); ++i) {
				tempsum += base.getBase(i+1).bitLength();
			}
